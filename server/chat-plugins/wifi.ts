	...
	change(key: string, value: string, user: User) {
		...
		if (force) {
			...
			if (!this.winner) {
				...
				this.phase = 'ended';
				this.clearTimer();
				this.room.modlog('GIVEAWAY WIN', undefined, undefined, undefined, undefined, undefined, `${this.winner.name} won ${this.giver.name}'s giveaway for a "${this.prize}" (OT: ${this.ot} TID: ${this.tid})`);
...
export class LotteryGiveaway extends Giveaway {
	...
	drawLottery() {
		...
		if (force) {
			...
			this.room.modlog('GIVEAWAY WIN', undefined, undefined, undefined, undefined, undefined, `${winnerNames} won ${this.giver.name}'s giveaway for "${this.prize}" (OT: ${this.ot} TID: ${this.tid})`);
			this.send(this.generateWindow(
...
export class GTSGiveaway {
	...
	stopDeposits() {
		...
		if (force) {
			...
			this.room.modlog('GTS FINISHED', undefined, undefined, undefined, undefined, undefined, `${this.giver.name} has finished their GTS giveaway for "${this.summary}"`);
